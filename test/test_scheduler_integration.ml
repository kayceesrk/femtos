...
let test_ivar_with_fifo () =
  ...
  let main () =
    ...
    Mux.Fifo.fork (fun () ->
      ...
      Mux.Fifo.yield () ; (* Let consumer start first *)
    ...
    Mux.Fifo.yield () ; (* Let other fibers run *)
    Printf.printf "Main: Test completed with result: %s\n"
      (match !result with Some v -> string_of_int v | None -> "None")
  in
...
let test_mixed_ivar_mvar () =
  Printf.printf "=== Testing mixed IVar and MVar operations with FIFO scheduler ===\n" ;
  ...
  let main () =
    ...
    Mux.Fifo.fork (fun () ->
      ...
      let config = Sync.Ivar.read config_ivar in
      Printf.printf "Worker2: Got config '%s', waiting for work from Worker1...\n" config ;
...
let () =
  Printf.printf "Running comprehensive IVar and MVar tests with FIFO scheduler...\n\n" ;
  test_ivar_with_fifo () ;
